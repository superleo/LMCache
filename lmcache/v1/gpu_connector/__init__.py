...
# Devices that have a working SGLang connector wired into the factory.
# CUDA uses ``SGLangGPUConnector`` (C++ ``lmc_ops`` kernels). MUSA uses
# ``SGLangMUSAConnector`` (pure-torch ``index_copy_`` / ``index_select``).
# XPU and HPU have no SGLang port today — their dispatch fails fast.
_SGLANG_SUPPORTED_DEVICES: frozenset[str] = frozenset({"cuda", "musa"})


def _validate_sglang_device() -> None:
    """Reject SGLang dispatch on devices without an SGLang connector.

    Raises:
        ValueError: When ``torch_device_type`` is not in
            :data:`_SGLANG_SUPPORTED_DEVICES`. The message points operators
            at either CUDA / MUSA (both supported) or vLLM as the
            recommended unblock for non-supported accelerators.
    """
    if torch_device_type in _SGLANG_SUPPORTED_DEVICES:
        return
    raise ValueError(
        f"SGLang engine on device '{torch_device_type}' is not supported: "
        f"LMCache ships no SGLang connector for this accelerator. "
        f"Supported SGLang devices: {sorted(_SGLANG_SUPPORTED_DEVICES)}. "
        f"Run SGLang on a supported accelerator, or use the vLLM integration."
    )
    ...
    if engine == EngineType.SGLANG:
        ...
        if torch_device_type == "musa":
            ...
            if config.use_layerwise:
                return SGLangLayerwiseMUSAConnector.from_metadata(
                    metadata,
                    use_gpu=use_gpu,
                    device=device,
            ...
            return SGLangMUSAConnector.from_metadata(
                metadata,
                use_gpu=use_gpu,
                device=device,
                chunk_size=chunk_size,
